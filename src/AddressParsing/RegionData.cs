...
namespace AddressParsing
{
    public static class RegionData
    {
        ...
        public static string ReadRegionsFile4()
        {
            var curpath = Path.Combine(Directory.GetCurrentDirectory(), "AddressParsingRegions.json");
            if (File.Exists(curpath))
            {
            ...
            using (Stream sm = Assembly.GetExecutingAssembly().GetManifestResourceStream($"{Assembly.GetExecutingAssembly().GetName().Name}.Internal.Regions4.json"))
            {
                if (sm == null)
                {
                    return string.Empty;
